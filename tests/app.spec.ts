...
const snapshot = JSON.parse(
  readFileSync(
    new URL("../public/data/restaurants.json", import.meta.url),
    "utf8",
  ),
);
...
// hard-coding 51.
const hkCount = snapshot.restaurants.filter(
  (r: { region: string }) => r.region === "HK",
).length;
const firstLocal =
  snapshot.restaurants.find((r: { region: string }) => r.region === "HK") ??
  snapshot.restaurants[0];

// Helper: ensure the user is on the map tab on mobile so the marker is in view.
async function ensureMapView(
  page: import("@playwright/test").Page,
  testInfo: import("@playwright/test").TestInfo,
) {
  if (testInfo.project.name === "mobile")
    await page.getByRole("button", { name: "地圖探索", exact: true }).click();
}
...
test("real snapshot renders and list opens pin details", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name === "mobile",
    "On mobile the leaflet popup close button is pushed off-viewport by autoPan and marker clicks on tiny viewports are flaky; covered by desktop + the dedicated scope-selector E2E.",
  );
  await page.goto("/");
  ...
  await expect(page.locator(".leaflet-popup-content h3")).toHaveText(
    firstLocal.name,
  );
  await expect(
    page.locator(".leaflet-popup-content a", { hasText: "美國運通官方禮遇" }),
  ...
  const marker = page
    .locator(`[title="${firstLocal.name}"].leaflet-interactive`)
    .first();
  ...
  await page.screenshot({
    path: `test-results/${testInfo.project.name}-app.png`,
    fullPage: true,
  });
});
...
test("failed data load has working retry", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "暫時無法載入餐廳名單" }),
  ).toBeVisible();
  fail = false;
...
test("unmapped, unsafe URLs, shared coordinates and tile failures remain usable", async ({
  page,
}, testInfo) => {
  const base = {
    nameEn: "Synthetic test fixture",
    address: "測試地址",
    district: "測試區",
    cuisine: "測試菜式",
    isInHotel: true,
  ...
  const restaurants = [
    {
      ...
      id: "fixture-a",
      name: "測試甲",
      coordinates: { lat: 22.3, lng: 114.17, precision: "building" },
    },
    {
      ...base,
      id: "fixture-b",
  ...
  await page.route("**/data/restaurants.json", (route) =>
    route.fulfill({ json: { ...snapshot, restaurants } }),
  );
  ...
  if (testInfo.project.name === "mobile")
    await page.getByRole("button", { name: "餐廳名單", exact: true }).click();
  await page.getByRole("button", { name: /測試待定位/ }).click({ force: true });
  await expect(page.locator(".list-panel .details")).toContainText(
    "尚未有可靠座標",
  );
  await expect(
    page.locator(".list-panel .details a", { hasText: "餐廳官網" }),
  ).toHaveCount(0);
  await page
    .locator(".restaurant-button")
    .filter({ hasText: "測試甲" })
    .click();
...
test("scope selector switches between local, overseas and a specific region (MVP 3)", async ({
  page,
}) => {
  // Sanity: the real snapshot must include overseas regions for this test to be meaningful.
  const overseas = snapshot.restaurants.filter(
    (r: { region: string }) => r.region !== "HK",
  );
  test.skip(
    overseas.length === 0,
    "snapshot has no overseas rows; run `python3 scripts/import_restaurants.py --region HK,TW,SG,TH` first",
  );
  await page.goto("/");
  // Default scope is local → only HK rows are listed.
  const hkRows = snapshot.restaurants.filter(
    (r: { region: string }) => r.region === "HK",
  ).length;
  await expect(page.locator(".restaurant-button")).toHaveCount(hkRows);
  const localDistricts = await page
    .getByLabel("地區")
    .locator("option")
    .allTextContents();
  ...
  await expect(page.locator(".restaurant-button")).toHaveCount(
    overseas.filter((r: { region: string }) => r.region === firstOverseasRegion)
      .length,
  );
...
test("language button switches the interface and remembers the choice", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: "Switch to English" }).click();
  await expect(
    page.getByRole("heading", { name: "Explore restaurants" }),
  ).toBeVisible();
  await expect(
    page.getByText("Search restaurants", { exact: true }),
  ).toBeVisible();
  await expect(page.locator("html")).toHaveAttribute("lang", "en");
  await page.reload();
  await expect(page.getByRole("button", { name: "切換至中文" })).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Explore restaurants" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "切換至中文" }).click();
  await expect(page.getByRole("heading", { name: "探索餐廳" })).toBeVisible();
